...
% Modified by LiYang_faruto 2015.03.03
% Add 
% 'XLabelString'
% 'YLabelString'
...
[FigShape,FigSize,FigStyle,FillStyle,DisplayOpt,TextColor,XVarNames,...
    YVarNames,ColorBar,GridOpt,...
    XLabelString,YLabelString] = parseInputs(varargin{:});
...
% figure('color','w',...
%     'units','normalized',...
%     'Position',[scrsz(3)*1/4 scrsz(4)*1/6 scrsz(3)*4/5 scrsz(4)]*3/4);
...
if ~isempty(XLabelString)
    H.XLabel.String = XLabelString;
end
if ~isempty(YLabelString)
    H.YLabel.String = YLabelString;
end
...
%% xlabel ylabel设置
xlabel = get(ha,'XLabel');
ylabel = get(ha,'YLabel');
if ~isempty(xlabel.String)
    tLen = floor(length(xtick)/2);
    xpos = xtick( tLen );
    tLen = floor(length(y)/2);
    ypos = 3*y( tLen );
    text(xpos,ypos,xlabel.String,...
        'Interpreter','none',...
        'color','k',...
        'HorizontalAlignment','center',...
        'FontWeight','Bold',...
...
if ~isempty(ylabel.String)
    tLen = floor(length(x)/2);
    xpos = 2*x( tLen );
    tLen = floor(length(ytick)/2);
    ypos = ytick( tLen );
    text(xpos,ypos,ylabel.String,...
        'Interpreter','none',...
        'color','k',...
        'HorizontalAlignment','center',...
        'FontWeight','Bold',...
...
function [FigShape,FigSize,FigStyle,FillStyle,DisplayOpt,TextColor,...
    XVarNames,YVarNames,ColorBar,GridOpt,XLabelString,YLabelString] = parseInputs(varargin)
...
[FigShape,FigSize,FigStyle,FillStyle,DisplayOpt,TextColor,XVarNames,...
    YVarNames,ColorBar,GridOpt,XLabelString,YLabelString] = parseArgs(pnames, dflts, varargin{:});
